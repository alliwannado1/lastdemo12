...
public class DanhMucService extends BasicService<DanhMuc> {
	...
	/*
	 * public List<DanhMuc> getList() { JPAQuery<DanhMuc> q = find(DanhMuc.class);
	 * 
	 * // truy vấn lấy ra danh mục cha và sắp xếp tăng dần
	 * q.where(QDanhMuc.danhMuc.trangThai.ne(core().TT_DA_XOA)).where(QDanhMuc.
	 * danhMuc.parent.isNull()); q.orderBy(QDanhMuc.danhMuc.soThuTu.asc());
	 * List<DanhMuc> list = q.fetch(); for (DanhMuc danhMuc : list) {
	 * danhMuc.loadChildren(); } return list; }
	 */
